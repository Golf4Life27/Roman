...
def stored_credentials():
    ...
    stops covering uploads. The metadata tools must not do that: they run
    against the same saved token and only need to know what it can already do,
    so the scopes are read out of the token file rather than asserted. No
    browser flow here either -- in CI there is nobody to click Allow.
    """
    from google.auth.transport.requests import Request
    from google.oauth2.credentials import Credentials
